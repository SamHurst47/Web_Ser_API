...
@router.get("/pace-trend", response_model=PaceTrendResponse)
def get_pace_trend(
    year: int = Query(...), location: str = Query(...),
    session_name: Optional[str] = Query(None),
    driver_numbers: Optional[List[int]] = Query(None),
...
@router.get("/ideal-laps", response_model=IdealLapsResponse)
def get_ideal_laps(
    year: int = Query(...), location: str = Query(...),
    session_name: Optional[str] = Query(None),
    driver_numbers: Optional[List[int]] = Query(None),
    ...
    # 1. Get the Overall "Franken-lap" for the session(s)
    ...
    # 2. Get individual driver ideal laps
...
@router.get("/true-pace", response_model=List[TruePaceCompare])
def get_true_pace(
    year: int = Query(...), location: str = Query(...),
    session_name: Optional[str] = Query(None),
    driver_numbers: Optional[List[int]] = Query(None),
